...
\begin{columns}[T]
	...
	\begin{column}{0.25\textwidth}
		...
		\begin{exampleblock}{En \forme{eXtensible Markup Language} (XML)}
			\inputminted{exemples-structures/xml.xml}
		\end{exampleblock}
	\end{column}
	\begin{column}{0.25\textwidth}
		\begin{exampleblock}{En \forme{YAML Ain't Markup Language}}
			\inputminted{exemples-structures/yaml.yaml}
		\end{exampleblock}
	\end{column}
	\begin{column}{0.25\textwidth}
		\begin{exampleblock}{En \forme{JavaScript Object Notation} (JSON)}
			\inputminted{exemples-structures/json.json}
		\end{exampleblock}
	\end{column}
\end{columns}
\end{frame}
...
\begin{frame}
	\begin{itemize}
		\item On peut mettre des \textbf{données} dans des \texbf{fichiers} qu'on décrypte ensuite.
		\item Mais on peut aussi \texbf{faire abstraction} des fichiers pour ne se focaliser que sur les \texbf{données} et leurs \texbf{relations}.
	\end{itemize}
\end{frame}

\begin{frame}
	\begin{itemize}
		\item On utilise alors un \texbf{gestionnaire de bases de données}.
		\item Qu'on manipule avec un \texbf{langage spécifique}.
