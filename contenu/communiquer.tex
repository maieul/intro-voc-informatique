...
% FIXME
% je pense que cette section est floue, beaucoup de technique, et à la fin je ne vois pas le message que tu veux qu'on en retienne
% 
% idem que la section précédente, j'aimerais voir une sorte de coupe à travers les niveaux d'abstraction, pour faire réfléchir les gens
% ex:
% - on veut transmettre une information à quelqu'un d'autre (choix du fond)
% - il faut choisir la forme (visuel, son, texte, présentation, roman, film)
% - et un medium de transmission (en personne, la poste, téléphone…)
% tu peux faire une analogie avec le code morse, qui marche sur plusieurs supports physiques: son, lumière visible, électricité
...
\begin{frame}
	\begin{itemize}
		...
		\begin{description}
			\item[Premier niveau (TLD)] : extensions (\url{.fr}, \url{.org}). % FIXME nombre limité, gérés par des entités (AFNIC…) imposant des règles -> explique .name, .org, .fr…
