...
\begin{slide}
	\begin{itemize}
		...
		\item Des langages \textbf{interprétés} de haut niveau:
                  % FIXME: la distinction compilé/interprété n'a aucun intérêt (et en l'occurence, Java est compilé)
                  % il vaut mieux illustrer le niveau d'abstraction dont on dispose avec chaque langage:
                  % très bas niveau: contrôle matériel de la machine
                  % bas niveau: contrôle précis des mécanismes d'exécution
                  % haut niveau: facilité d'expression d'un algorithme, portabilité
                  % sql, prolog: facilité d'expression d'un problème et de sa solution
		\begin{itemize}
			\item Java
			\item PHP
			\item Python
